...
module.exports = class FtxInterface {

   async ftxOrder(market, quantity, side) {
      ...
      return new Promise((resolve, reject) => {
         request({ headers, uri, method: 'POST', body: query, json: true }, function (err, res, ticket) {
            ...
            if (ticket && ticket.result && ticket.result.id) {
               resolve(`Order confirmed: ${ticket.result.id}`);
            } else {
               console.log(ticket);
               resolve(`Order FAILED`)
            }
         });
      });
   }
   ...
   /*
   ftxTrailingStop(market, quantity, stop) {
      const ts = new Date().getTime();
      const query = {
         market: market,
         side: 'sell',
